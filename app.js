...
function renderMetrics(target, stats) {
  ...
  target.innerHTML = [
    ...
    metric("本塁打", formatNumber(stats.hr), "", "hr"),
    metric("打点", formatNumber(stats.rbi), "", "rbi"),
    metric("得点", formatNumber(stats.runs), "", "runs"),
    metric("四球", formatNumber(stats.bb), "", "bb"),
    metric("死球", formatNumber(stats.hbp), "", "hbp"),
    ...
    metric("犠打", formatNumber(stats.sh), "", "sac-bunt"),
    ...
    metric("失策出塁", formatNumber(stats.errorsReached), "", "errors-reached"),
    metric("犠飛", formatNumber(stats.sf), "", "sac-fly"),
  ].join("");
}
  ...
  const cells = COURSE_GRID.map((course) => {
    ...
    return `
      <div class="course-cell ${courseCellTone(avg, row.ab)}">
        ...
        <span class="course-detail">長打 ${row.long} / 三振 ${row.k}</span>
      </div>
    `;
  }).join("");

  els.courseSupplementChart.innerHTML = `
    <div class="course-grid" role="img" aria-label="コース別成績。表示は打数-安打、打率、長打、三振です。">
      ${cells}
    </div>
    <div class="course-supplement-footer">
      <p class="muted">表示は「打数-安打 / 打率 / 長打・三振」です。9分割以外のコースは ${stats.otherAb}打数あります。</p>
